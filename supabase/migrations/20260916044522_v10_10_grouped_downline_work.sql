-- v10.10: additive read-only APIs; no policies, settlement routines or order snapshots change.
begin;
set local lock_timeout='5s';
do $$ begin
  if not exists(select 1 from public.app_schema_versions where version='v10.9.0') then raise exception 'v10.9.0 required'; end if;
  if exists(select 1 from public.app_schema_versions where version='v10.10.0') then raise exception 'v10.10.0 already applied'; end if;
end $$;

-- Private, non-API scope resolver. UNION deduplicates IDs, including malformed cycles.
create schema if not exists spark_private;
revoke all on schema spark_private from public,anon,authenticated;
create function spark_private.downline_members_v1010()
returns table(id uuid,username text,group_name text)
language plpgsql stable security invoker set search_path='' as $$
declare actor public.profiles;
begin
  select * into actor from public.profiles p where p.id=auth.uid();
  if actor.id is null or actor.role not in ('agency','distributor') or actor.role is null
     or coalesce(actor.is_operations_manager,false)
     or actor.approval_status is distinct from 'approved' or not coalesce(actor.active,false) then
    raise exception using errcode='42501',message='승인된 활성 대행사·총판만 하위 작업을 조회할 수 있습니다.';
  end if;
  return query with recursive descendants(member_id) as (
    select actor.id
    union
    select p.id from public.profiles p join descendants d on p.sponsor_id=d.member_id
  ) select p.id,p.username,coalesce(nullif(trim(p.group_name),''),'미지정 그룹')
    from descendants d join public.profiles p on p.id=d.member_id
    where p.id<>actor.id and p.role in ('agency','distributor') and not coalesce(p.is_operations_manager,false);
  -- Inactive descendants remain visible for historical work, just like existing manager reads.
  -- Deleted profiles are absent; no active/approval test interrupts intermediate links.
end $$;
revoke all on function spark_private.downline_members_v1010() from public,anon,authenticated;

create or replace function public.get_manager_agency_orders_v1010(
...
begin
  ...
  with scoped as (
    select
      ...
      coalesce(nullif(trim(mp.group_name),''),'미지정 그룹') as current_group_name,
      o.program_type,
      o.store_name,
      o.keyword,
      o.mid,
    ...
    where (p_agency_id is null or l.registrant_id = p_agency_id)
      ...
      and (
        ...
        or l.current_group_name ilike '%' || trim(p_query) || '%'
...
revoke all on function public.get_manager_agency_orders_v1010(uuid,text,text[],text,text,date,date,integer,integer,text) from public, anon, authenticated;
grant execute on function public.get_manager_agency_orders_v1010(uuid,text,text[],text,text,date,date,integer,integer,text) to authenticated;

create or replace function public.get_manager_managed_orders_v1010(
  p_agency_id uuid default null,
  p_program_type text default null,
  p_order_statuses text[] default null,
  p_settlement_status text default null,
...
begin
  ...
  with scoped as (
    ...
    where mp.manager_id = auth.uid()
      and (p_agency_id is null or mp.id = p_agency_id)
      and o.archived_at is null
  ), labeled as (
    select
      s.*,
...
revoke all on function public.get_manager_managed_orders_v1010(uuid,text,text[],text,text,date,date,integer,integer,text) from public, anon, authenticated;
grant execute on function public.get_manager_managed_orders_v1010(uuid,text,text[],text,text,date,date,integer,integer,text) to authenticated;

create function public.get_manager_agency_folders_v1010(
...
begin
  ...
  with managed as (
    select id,username,coalesce(nullif(trim(group_name),''),'미지정 그룹') group_name from public.profiles where manager_id=auth.uid() and (p_agency_id is null or id=p_agency_id)
  ), financials as (
    select o.id, o.created_by, o.status::text order_status, o.created_at,
      md5(o::text || coalesce(ps.step_revision,'')) revision,
      ...
      and (nullif(trim(p_query),'') is null or mp.group_name ilike '%'||trim(p_query)||'%' or mp.username ilike '%'||trim(p_query)||'%'
    ...
    left join lateral (
      select md5(coalesce(string_agg(s::text,',' order by s.id),'')) step_revision, count(*) total_steps, count(*) filter(where s.confirmed_at is not null) confirmed_steps,
    ...
    select mp.id as agency_id,mp.username,mp.group_name,
      ...
      max(f.created_at) last_order_at, md5(mp.username || mp.group_name || coalesce(string_agg(f.revision,',' order by f.id),'')) revision
    from managed mp left join financials f on f.created_by=mp.id group by mp.id,mp.username,mp.group_name
  ), filtered as (
    select * from grouped g where g.matched_order_count>0 or (
      ...
      and (nullif(trim(p_query),'') is null or g.group_name ilike '%'||trim(p_query)||'%' or g.username ilike '%'||trim(p_query)||'%')
    )
    ...
    'agencies',coalesce((select jsonb_agg(jsonb_build_object(
      'revision',revision,'agencyId',agency_id,'username',username,'groupName',group_name,'totalOrderCount',total_order_count,
...
revoke all on function public.get_manager_agency_folders_v1010(integer,integer,text,text,uuid,text,text[],text,date,date) from public,anon,authenticated;
grant execute on function public.get_manager_agency_folders_v1010(integer,integer,text,text,uuid,text,text[],text,date,date) to authenticated;


create or replace function public.get_downline_orders_v1010(
  p_agency_id uuid default null,
  p_program_type text default null,
  p_order_statuses text[] default null,
  p_settlement_status text default null,
...
declare
  ...
  v_page_size integer := least(greatest(coalesce(p_page_size, 50), 1), 500);
begin
  -- Always check caller even when the hierarchy is empty.
  perform 1 from spark_private.downline_members_v1010();
  if p_agency_id is not null and not exists(select 1 from spark_private.downline_members_v1010() d where d.id=p_agency_id) then
    raise exception using errcode='42501',message='현재 sponsor 하위 대행사만 조회할 수 있습니다.';
  end if;
  if p_program_type is not null and p_program_type not in ('spark', 'spark_plus', 'spark_s', 'spark_s_plus') then
    raise exception '지원하지 않는 프로그램 필터입니다.';
  end if;
  ...
  with scoped as (
    select
      ...
      coalesce(ps.confirmed_steps, 0)::bigint as confirmed_steps,
      coalesce(ps.total_steps, 0)::bigint as total_steps
    from spark_private.downline_members_v1010() mp
    join public.orders o on o.created_by = mp.id
    left join lateral (
      select
        count(*) filter (where step.confirmed_at is not null)::bigint as confirmed_steps,
    ...
    where (p_agency_id is null or mp.id = p_agency_id)
      and (p_group_name is null or mp.group_name=p_group_name)
      and o.archived_at is null
  ), labeled as (
    select
      s.*,
...
revoke all on function public.get_downline_orders_v1010(uuid,text,text[],text,text,date,date,integer,integer,text,text) from public, anon, authenticated;
grant execute on function public.get_downline_orders_v1010(uuid,text,text[],text,text,date,date,integer,integer,text,text) to authenticated;

create function public.get_downline_agency_overview_v1010(
  p_page integer default 1, p_page_size integer default 20,
  p_query text default null, p_sort text default 'in_progress',
  p_agency_id uuid default null, p_program_type text default null,
  p_order_statuses text[] default null, p_settlement_status text default null,
  p_start_date_from date default null, p_start_date_to date default null, p_group_name text default null
) returns jsonb language plpgsql stable security definer set search_path = '' as $$
declare
  v_actor public.profiles;
  v_page integer := greatest(coalesce(p_page,1),1);
...
begin
  ...
  if p_sort is null or p_sort not in ('in_progress','settlement_waiting','recent','username') then raise exception '지원하지 않는 폴더 정렬입니다.'; end if;
  with managed as (
    select * from spark_private.downline_members_v1010() d where (p_agency_id is null or d.id=p_agency_id) and (p_group_name is null or d.group_name=p_group_name)
  ), financials as (
    select o.id, o.created_by, o.status::text order_status, o.created_at,
      md5(o::text || coalesce(ps.step_revision,'')) revision,
      -- Caller receivable only: never sum the whole chain or substitute order total.
      ps.waiting_amount,
      ps.completed_amount,
      (p_program_type is null or o.program_type=p_program_type)
      and (p_order_statuses is null or o.status::text=any(p_order_statuses))
    ...
    left join lateral (
      select md5(coalesce(string_agg(s::text,',' order by s.id),'')) step_revision, count(*) total_steps, count(*) filter(where s.confirmed_at is not null) confirmed_steps,
        count(*) filter(where s.payee_id=auth.uid()) direct_count,
        coalesce(sum(s.total_amount) filter(where s.payee_id=auth.uid() and s.confirmed_at is null),0)::bigint waiting_amount,
        coalesce(sum(s.total_amount) filter(where s.payee_id=auth.uid() and s.confirmed_at is not null),0)::bigint completed_amount
      from public.payment_steps s where s.order_id=o.id
    ) ps on true where o.archived_at is null
  ), grouped as (
    select mp.id as agency_id,mp.username,mp.group_name,
...
revoke all on function public.get_downline_agency_overview_v1010(integer,integer,text,text,uuid,text,text[],text,date,date,text) from public,anon,authenticated;
grant execute on function public.get_downline_agency_overview_v1010(integer,integer,text,text,uuid,text,text[],text,date,date,text) to authenticated;


create function public.get_downline_group_overview_v1010(
  p_page integer default 1, p_page_size integer default 20,
  p_query text default null, p_sort text default 'in_progress',
  p_agency_id uuid default null, p_program_type text default null,
  p_order_statuses text[] default null, p_settlement_status text default null,
...
begin
  ...
  with managed as (
    ...
    select group_name, count(*)::bigint agency_count,
      sum(total_order_count)::bigint total_order_count,sum(matched_order_count)::bigint matched_order_count,
      sum(in_progress_count)::bigint in_progress_count,sum(running_count)::bigint running_count,
      sum(expired_count)::bigint expired_count,sum(stopped_count)::bigint stopped_count,
      sum(waiting_amount)::bigint waiting_amount,sum(completed_amount)::bigint completed_amount,
      max(last_order_at) last_order_at, md5(string_agg(revision,',' order by agency_id)) revision from filtered group by group_name
  ), totals as (select count(*) n from group_totals), bounds as (
    select n, greatest(1,ceil(n::numeric/v_size)::integer) pages,
      least(v_page,greatest(1,ceil(n::numeric/v_size)::integer)) page from totals
  ), ranked as (
    select g.*,row_number() over(order by
      ...
      case when p_sort in ('in_progress','settlement_waiting') then waiting_amount end desc,
      case when p_sort in ('in_progress','settlement_waiting','recent') then last_order_at end desc nulls last,
      lower(group_name),group_name) position from group_totals g
  ), paged as (
    select r.* from ranked r cross join bounds b where r.position>(b.page-1)::bigint*v_size and r.position<=b.page::bigint*v_size
  ) select jsonb_build_object('page',b.page,'pageSize',v_size,'totalPages',b.pages,'groupCount',b.n,
    'groups',coalesce((select jsonb_agg(jsonb_build_object(
      'revision',revision,'groupName',group_name,'agencyCount',agency_count,'totalOrderCount',total_order_count,
      'matchedOrderCount',matched_order_count,'inProgressCount',in_progress_count,
      'runningCount',running_count,'expiredCount',expired_count,'stoppedCount',stopped_count,
      'settlementWaitingAmount',waiting_amount,'settlementCompletedAmount',completed_amount,'lastOrderAt',last_order_at
    ) order by position) from paged),'[]'::jsonb)) into v_result from bounds b;
...
revoke all on function public.get_downline_group_overview_v1010(integer,integer,text,text,uuid,text,text[],text,date,date,text) from public,anon,authenticated;
grant execute on function public.get_downline_group_overview_v1010(integer,integer,text,text,uuid,text,text[],text,date,date,text) to authenticated;


insert into public.app_schema_versions(version,description) values('v10.10.0','Read-only sponsor descendant group/agency/order views and current profile group labels');
