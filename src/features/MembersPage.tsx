...
export function MembersPage({ user, members, onReview }: {
  ...
  return (
    <div className="page-stack members-page-stack">
      <PageHeader
        title="회원관리"
        subtitle={isAdmin ? '전체 회원의 그룹, 추천 관계, 유형과 단가를 관리합니다.' : '내 추천 코드로 가입한 대행사를 승인하고 단가를 관리합니다.'}
      />
      ...
      <section className="panel members-panel fill-panel">
        ...
        {selected && <div className="member-editor">
          ...
          <div className="member-editor-grid">
            ...
            {isAdmin && <label><span>관리자용 그룹명</span><input value={groupName} onChange={(event) => setGroupName(event.target.value)} placeholder={selected.sponsorId ? '상위 그룹명 자동 상속' : '업체 식별용 그룹명'} /></label>}
            <div className="member-static-info"><span>상위 회원</span><strong>{selected.sponsorUsername || '관리자 직속'}</strong></div>
          </div>
          {adminCannotReviewReferralPending && <p className="inline-message">이 회원의 가입 승인 요청은 직접 추천인인 <strong>{selected.sponsorUsername}</strong> 회원에게 전달되었습니다.</p>}
