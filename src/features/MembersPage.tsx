...
export function MembersPage({ user, members, onReview, onAssignManager, onBulkAssignManager, onCheckDeletion, onDeleteMember, onResetPassword }: {
  ...
  const renderMemberEditor = (surface: 'desktop' | 'mobile') => (
    selected ? <div className="member-editor" id={`member-editor-${surface}-${selected.id}`}>
      <div className="member-editor-title"><div><span>선택 회원</span><strong>{selected.username}</strong></div><button className="icon-button" aria-label="회원 편집 닫기" disabled={saving} onClick={() => setSelectedId(null)}>×</button></div>
