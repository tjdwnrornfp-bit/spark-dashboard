...
export function MembersPage({ user, members, onReview }: {
  ...
  const save = async (approvalStatus: 'approved' | 'rejected') => {
    ...
    try {
      await onReview({
        ...
        prices: approvalStatus === 'approved' ? numericPrices : emptyPrices(selected),
        approvalStatus,
  ...
  return (
    <div className="page-stack members-page-stack">
      <PageHeader
        title="회원관리"
        subtitle={isAdmin ? '전체 회원의 그룹, 추천 관계, 유형과 프로그램별 단가를 관리합니다.' : '내 추천 코드로 가입한 하위 대행사를 승인하고 프로그램별 단가를 관리합니다.'}
      />
      ...
      <section className="panel members-panel fill-panel">
        ...
        <div className="desktop-table"><table className="members-table"><thead><tr><th>아이디</th>{isAdmin && <th>그룹명</th>}{isAdmin && <th>추천인</th>}<th>회원유형</th><th>스파크</th><th>스파크 +</th><th>스파크S</th><th>승인상태</th><th>가입 신청일</th><th>관리</th></tr></thead><tbody>{visible.map((member) => { const memberPrices = getProgramPriceMap(member); return <tr key={member.id} className={selectedId === member.id ? 'selected-row' : ''}><td><strong>{member.username}</strong><small className="table-subtext">코드 {member.referralCode || '-'}</small></td>{isAdmin && <td>{member.groupName || '-'}</td>}{isAdmin && <td>{member.sponsorUsername || '관리자 직속'}</td>}<td>{member.role === 'distributor' ? '총판' : member.role === 'agency' ? '대행사' : '-'}</td><td>{memberPrices.spark > 0 ? formatWon(memberPrices.spark) : '-'}</td><td>{memberPrices.spark_plus > 0 ? formatWon(memberPrices.spark_plus) : '-'}</td><td>{memberPrices.spark_s > 0 ? formatWon(memberPrices.spark_s) : '-'}</td><td><ApprovalBadge status={member.approvalStatus} /></td><td>{formatDateTime(member.requestedAt)}</td><td><button className="dark-small-button" onClick={() => open(member)}>{member.approvalStatus === 'approved' ? '수정' : '검토'}</button></td></tr>})}</tbody></table></div>
        <div className="mobile-member-list">{visible.map((member) => { const memberPrices = getProgramPriceMap(member); return <article key={member.id}><div><strong>{member.username}</strong><ApprovalBadge status={member.approvalStatus} /></div><p>{isAdmin ? `${member.sponsorUsername ? `추천인 ${member.sponsorUsername}` : '관리자 직속'} · ` : ''}{member.role === 'distributor' ? '총판' : '대행사'}</p><p>스파크 {memberPrices.spark > 0 ? formatWon(memberPrices.spark) : '-'} · 스파크+ {memberPrices.spark_plus > 0 ? formatWon(memberPrices.spark_plus) : '-'} · 스파크S {memberPrices.spark_s > 0 ? formatWon(memberPrices.spark_s) : '-'}</p>{isAdmin && <p>그룹 {member.groupName || '-'}</p>}<button className="secondary-button small" onClick={() => open(member)}>회원 검토</button></article>})}</div>
        ...
        {selected && <div className="member-editor">
          ...
          <div className="member-editor-grid">
            ...
            <label><span>스파크 단가</span><div className="input-unit"><input type="number" min={isAdmin ? 1 : parentPrices.spark + 1} step="1" value={prices.spark || ''} onChange={(event) => setPrices((current) => ({ ...current, spark: Number(event.target.value) || 0 }))} /><span>원</span></div></label>
            <label><span>스파크 + 단가</span><div className="input-unit"><input type="number" min={isAdmin ? 1 : parentPrices.spark_plus + 1} step="1" value={prices.spark_plus || ''} onChange={(event) => setPrices((current) => ({ ...current, spark_plus: Number(event.target.value) || 0 }))} /><span>원</span></div></label>
            <label><span>스파크S 단가</span><div className="input-unit"><input type="number" min={isAdmin ? 1 : parentPrices.spark_s + 1} step="1" value={prices.spark_s || ''} onChange={(event) => setPrices((current) => ({ ...current, spark_s: Number(event.target.value) || 0 }))} /><span>원</span></div></label>
