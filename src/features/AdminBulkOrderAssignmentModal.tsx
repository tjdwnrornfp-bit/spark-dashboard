...
export function AdminBulkOrderAssignmentModal({ onLoadMembers, onAssign, onClose }: {
  ...
  const upload = async (file?: File) => {
    if (!file) return
    try { const parsed = readAssignmentWorkbook(await file.arrayBuffer()); setRows(parsed); setResults([]); setError(''); setRequestId(crypto.randomUUID()); setStarted(false) }
