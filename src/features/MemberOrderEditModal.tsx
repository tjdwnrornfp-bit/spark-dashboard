...
export function MemberOrderEditModal({ order, programOnly, onPreview, onApply, onClose }: {
  ...
  return <Modal title={programOnly ? '프로그램 변경' : '내 작업 수정'} description="입금 확인 전 작업만 수정할 수 있습니다. 프로그램 유지 시 기존 단가, 프로그램 변경 시 대상 프로그램의 현재 승인 단가를 적용합니다." onClose={() => { if (!busy) onClose() }} footer={<><button className="secondary-button" disabled={busy} onClick={onClose}>취소</button><button className="primary-button" disabled={busy || Boolean(startDateRestrictionError(draft.startDate, restrictions.rules, order.startDate)) || reason.trim().length < 2 || Boolean(preview && !preview.allowed) || Boolean(preview && warning && !accepted)} onClick={() => void run()}>{busy ? '확인 중…' : preview?.allowed ? '수정 적용' : '변경 영향 확인'}</button></>}>
    ...
    <fieldset disabled={busy} className="assignment-fields">
      ...
      {!programOnly && fields.map(([key, label]) => <label className="field" key={key}><span>{label}</span>{key === 'startDate' ? <StartDateInput restrictions={restrictions} originalDate={order.startDate} value={draft.startDate} onChange={(value) => change('startDate', value)} /> : <input value={draft[key]} type={['dailyShots', 'operationDays'].includes(key) ? 'number' : 'text'} min={1} step={1} maxLength={key === 'memo' ? 300 : ['storeName', 'keyword'].includes(key) ? 50 : undefined} onChange={(e) => change(key, e.target.value)} />}</label>)}
