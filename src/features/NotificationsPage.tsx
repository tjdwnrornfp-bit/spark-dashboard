...
export function NotificationsPage({ filterValue, onFilterChange, unreadTotal, notificationTotal, user, notifications, hasMore, loadingMore, onLoadMore, onRead, onReadAll, onDelete, onDeleteAll }: {
  filterValue?: 'all' | 'unread'
  ...
  const [localFilter, setLocalFilter] = useState<'all' | 'unread'>('all')
  ...
  const unread = unreadTotal ?? allVisible.filter((item) => !item.read).length
  ...
  return (
    <div className="page-stack notifications-page-stack">
      ...
      <section className="panel notification-panel fill-panel">
        <div className="notification-tabs"><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>전체 <span>{notificationTotal ?? allVisible.length}</span></button><button className={filter === 'unread' ? 'active' : ''} onClick={() => setFilter('unread')}>읽지 않음 <span>{unread}</span></button></div>
        ...
        {hasMore && <div className="notification-load-more"><button className="secondary-button" disabled={loadingMore} onClick={() => void onLoadMore().catch((error) => window.alert(getErrorMessage(error)))}>{loadingMore ? '불러오는 중...' : '이전 알림 더보기'}</button></div>}
      </section>
    </div>
  )
}
