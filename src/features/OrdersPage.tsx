...
const BULK_HEADERS = ['상호명', '대표키워드', '플레이스URL', '일일수량', '구동일수', '메모']
...
export function OrdersPage({ user, orders, settings, now, onCreateOrder, onCreateOrdersBulk, onStatusChange }: {
  ...
  const downloadBulkTemplate = () => {
    ...
    worksheet['!cols'] = [{ wch: 30 }, { wch: 30 }, { wch: 70 }, { wch: 14 }, { wch: 14 }, { wch: 40 }]
    const workbook = utils.book_new()
    ...
    <div className="page-stack orders-page-stack">
      ...
      {user.role !== 'admin' && formOpen && <section className="panel intake-form-panel">
        ...
        <div className="form-grid compact-form">
          ...
          <Field label="일일 구동 수량" required error={errors.dailyShots}><div className="input-unit"><input type="number" min="1" step="1" value={draft.dailyShots} onChange={(event) => updateDraft('dailyShots', event.target.value)} /><span>타</span></div></Field>
