...
function buildLocalPaymentSteps(order: Order, members: User[]): PaymentStep[] {
  ...
  while (payer && payer.role !== 'admin' && admin) {
    ...
    steps.push({ id: crypto.randomUUID(), orderDbId: order.dbId ?? order.id, orderNumber: order.id, storeName: order.storeName, stepOrder, payerId: payer.id, payerUsername: payer.username, payeeId: payee.id, payeeUsername: payee.username, unitPrice: payer.pricePerShot, ...amount, confirmedAt: null, createdAt: order.createdAt })
...
export default function App() {
  ...
  const [localSessionUserId, setLocalSessionUserId] = useLocalStorage<string | null>('spark-session-v6', null)
  const [localMembers, setLocalMembers] = useLocalStorage<User[]>('spark-members-v6', DEMO_USERS)
  const [localOrders, setLocalOrders] = useLocalStorage<Order[]>('spark-orders-v6', initialDemoOrders)
  const [localPaymentSteps, setLocalPaymentSteps] = useLocalStorage<PaymentStep[]>('spark-payment-steps-v6', makeDemoPaymentSteps(initialDemoOrders))
  const [localNotifications, setLocalNotifications] = useLocalStorage<NotificationItem[]>('spark-notifications-v6', DEMO_NOTIFICATIONS)
  const [localNotices, setLocalNotices] = useLocalStorage<Notice[]>('spark-notices-v6', DEMO_NOTICES)
  const [localSettings, setLocalSettings] = useLocalStorage<AppSettings>('spark-settings-v6', DEFAULT_SETTINGS)
  ...
  const register = async (draft: SignupDraft) => {
    ...
    const member: User = { id, username, passwordHash: await hashPassword(draft.password), role: sponsor ? 'agency' : null, approvalStatus: 'pending', pricePerShot: 0, active: false, requestedAt: nowIso, approvedAt: null, updatedAt: nowIso, sponsorId: sponsor?.id ?? null, sponsorUsername: sponsor?.username ?? null, referralCode: makeReferralCode(id), groupName: sponsor?.groupName ?? '', hierarchyDepth: sponsor ? sponsor.hierarchyDepth + 1 : 0, bank: '', accountNumber: '', accountHolder: '' }
  ...
  const handleCreateOrder = async (draft: OrderDraft): Promise<Order> => {
    ...
    setLocalOrders((current) => [...current, order]); setLocalPaymentSteps((current) => [...current, ...buildLocalPaymentSteps(order, localMembers)])
  ...
  const handleCreateOrdersBulk = async (drafts: OrderDraft[]): Promise<Order[]> => {
    if (isSupabaseConfigured) { const created = await createRemoteOrdersBulk(drafts); setRemoteOrders((current) => [...current, ...created]); void refreshRemote(); return created }
    const created: Order[] = []
    let existing = [...localOrders]
    for (const draft of drafts) { const order = createOrder(user!, draft, localSettings, existing, new Date()); existing.push(order); created.push(order) }
    setLocalOrders(existing); setLocalPaymentSteps((current) => [...current, ...created.flatMap((order) => buildLocalPaymentSteps(order, localMembers))])
    const admin = localMembers.find((member) => member.role === 'admin')
  ...
  const handleMemberReview = async (params: MemberReviewInput) => {
    if (!user) throw new Error('로그인이 필요합니다.')
    if (isSupabaseConfigured) { const updated = await reviewRemoteMember({ memberId: params.member.id, role: params.role, pricePerShot: params.pricePerShot, approvalStatus: params.approvalStatus, groupName: params.groupName }); setRemoteMembers((current) => current.map((member) => member.id === updated.id ? updated : member)); return }
    const target = params.member
    if (user.role !== 'admin' && target.sponsorId !== user.id) throw new Error('직접 추천한 회원만 관리할 수 있습니다.')
    if (user.role !== 'admin' && params.approvalStatus === 'approved' && params.pricePerShot <= user.pricePerShot) throw new Error('하위 단가는 내 단가보다 높아야 합니다.')
    const nowIso = new Date().toISOString()
    const updated: User = { ...target, role: target.sponsorId ? 'agency' : params.role, pricePerShot: params.approvalStatus === 'approved' ? params.pricePerShot : target.pricePerShot, groupName: target.sponsorId ? (localMembers.find((member) => member.id === target.sponsorId)?.groupName ?? target.groupName) : params.groupName, approvalStatus: params.approvalStatus, active: params.approvalStatus === 'approved', approvedAt: params.approvalStatus === 'approved' ? target.approvedAt ?? nowIso : target.approvedAt, updatedAt: nowIso }
    setLocalMembers((current) => current.map((member) => member.id === target.id ? updated : member)); setLocalNotifications((current) => [{ id: crypto.randomUUID(), createdAt: nowIso, userId: target.id, role: 'all', title: params.approvalStatus === 'approved' ? '회원가입 승인 완료' : '회원가입 반려', message: params.approvalStatus === 'approved' ? `대행사 회원으로 승인되었습니다. 1타당 단가는 ${params.pricePerShot}원입니다.` : '회원가입 신청이 반려되었습니다.', read: false }, ...current])
  }

  const handleAccountChange = async (account: AccountDraft) => {
    if (!user) throw new Error('로그인이 필요합니다.')
    if (isSupabaseConfigured) { const updated = await saveRemoteAccount(account); setRemoteUser(updated); setRemoteMembers((current) => current.map((member) => member.id === updated.id ? updated : member)); return }
  ...
  const handlePasswordChange = async (currentPassword: string, nextPassword: string) => { if (!user) throw new Error('로그인이 필요합니다.'); if (isSupabaseConfigured && supabase) { const { error } = await supabase.auth.updateUser({ password: await passwordToAuthSecret(nextPassword), current_password: await passwordToAuthSecret(currentPassword) }); if (error) throw error; return }; if (await hashPassword(currentPassword) !== user.passwordHash) throw new Error('현재 비밀번호가 올바르지 않습니다.'); const nextHash = await hashPassword(nextPassword); setLocalMembers((current) => current.map((member) => member.id === user.id ? { ...member, passwordHash: nextHash, updatedAt: new Date().toISOString() } : member)) }
